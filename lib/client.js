(function () {
  window.isEsbuildDevServerSetup = false
  ...
  function onLoad() {
    if (!window.SockJS || !window.ReactErrorOverlay || window.isEsbuildDevServerSetup) return;
    ...
    var result = JSON.parse(event.data);
      ...
      if (result.errors.length) {
        ...
        return reo.reportBuildError(errorMessage(result.errors[0]));
      }
